...
- [x] Step 8：Portal Workflow Builder 与 Jenkins 调度入口
...
- [Step 8：Portal Workflow Builder 与 Jenkins 调度入口](steps/step-08-platform-portal-workflow-builder-dispatch.md)
...
- `internal_tools` 内部模块
- orchestrator handler 直接调用内部 service；Portal standalone 工具 run 通过 `internal_tools.tool_runner` 调用同一套 service
