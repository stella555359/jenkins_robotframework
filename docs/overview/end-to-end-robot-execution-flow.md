...
3. 对默认 agent-local-key 模式，checkout-sources.sh 在 agent 上使用 `ROBOTWS_GIT_SSH_KEY_PATH` / `TESTLINE_CONFIGURATION_GIT_SSH_KEY_PATH` 调用 git
4. 只有显式指定 `credential_kind=sshagent` 时，才会从 source-checkout.json 提取 credential IDs 并进入 `sshagent(credentials) { ... }`
