...
- 我先在本文档里给出本 step 的目标、代码示例和流程图
- 然后我直接把对应代码落到 `platform-api/` 源码目录
- 你以 review 的方式看改动、对照文档学习
- 你 review 完后，如果有不懂的点再提问，我再补解释
- 每个 step 收尾时，我再围绕本 step 出几道小问题给你回答，帮助确认你是否真正理解

补充说明：

- 已经写好的 `Step 1` 逐行解释先保留，作为历史学习记录
- 从当前开始，后续 step 默认不再写“逐行解释”
- 如果你 review 代码时对某个语法、某一行或某一层调用关系不理解，再单独提问，我把问答补到“相关问题记录”
...
- [x] 第 3 步：加入基础配置层
- [x] 第 4 步：加入 run 相关 schema
- [x] 第 5 步：实现 `POST /api/runs`
- [x] 第 6 步：接入 SQLite
- [x] 第 7 步：实现 run 持久化
...
- `docs/platform-api_step_by_step.md`
