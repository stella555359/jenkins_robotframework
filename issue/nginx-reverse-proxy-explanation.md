...
可以把 Nginx 理解成“前台接待”，把 Jenkins、platform-api、kpi-portal 理解成“后面的实际办事窗口”。
...
后面如果再挂上 platform-api 和 kpi-portal，也还是同一套路：
...
4. 为什么后面 platform-api 和 kpi-portal 也可以继续复用同一套 Nginx 入口
...
Nginx
	...
	|-- /api/  ->  http://127.0.0.1:8000/           -> platform-api
...
1. `systemctl status platform-api` / `kpi-portal`
2. `ss -lntp | grep :8000` / `:8001`
3. `curl http://127.0.0.1:8000/health` / `:8001/health`
4. `systemctl status nginx`
5. `curl -k -I https://127.0.0.1/api/health`
...
| `/api/health` 不通 | Portal 一屏版 | 先看 platform-api 服务、8000 端口和本机健康检查 |
...
2. 如果是 `https://IP/api/...` 或 `https://IP/kpi/...` 返回 404：
...
| `/api/` 或 `/kpi/` 返回 `502` | Portal 一屏版 |
| Jenkins 登录跳转异常 / 资源 404 / URL 很怪 | Jenkins 一屏版 |
| `/api/health` 不通 | Portal 一屏版 |
